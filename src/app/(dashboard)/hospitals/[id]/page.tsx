...
import { useGetHospitalQuery } from '@/lib/api/hospitalsApi';
import { useCreateHospitalKycRecordMutation } from '@/lib/api/kycApi';
import HospitalAppointments from '@/components/dashboard/HospitalAppointments';
...
const HospitalProfilePage = () => {
    ...
    // Get hospital data
    const { data: hospital, isLoading: hospitalLoading, error: hospitalError } = useGetHospitalQuery(hospitalId);

    // KYC mutation
    const [createHospitalKycRecord, { isLoading: isUpdatingKyc }] = useCreateHospitalKycRecordMutation();
    ...
    const confirmStatusChange = async () => {
        ...
        try {
            await createHospitalKycRecord({
                hospital: hospitalId,
                status: pendingStatus,
                reason: rejectionReason || undefined,
            }).unwrap();

            toast.success(`Hospital KYC status updated to ${pendingStatus.toLowerCase()}`);
    ...
    const SidebarContent = () => {
        if (hospitalLoading) {
            return <div className="text-center py-8">Loading hospital information...</div>;
        }

        if (hospitalError || !hospital) {
            return <div className="text-center py-8 text-red-600">Error loading hospital information</div>;
        }
                    ...
                    <CardContent className="pt-6">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="flex items-center gap-2 mb-1 flex-wrap justify-center">
                                <h2 className="text-lg sm:text-xl font-semibold text-center">{hospital.name}</h2>
                                {getStatusBadge(hospital.kyc_status)}
                            </div>
                            <p className="text-muted-foreground text-sm mb-6">
                                {formatSpecialties(hospital.specialties)}
                            </p>
                            ...
                            <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:gap-8 w-full py-4 sm:py-6 border-t border-b">
                                <div className="text-center">
                                    ...
                                    <div className="text-muted-foreground text-xs sm:text-sm">Doctors</div>
                                </div>
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                        <Calendar className="w-4 h-4 text-purple-600" />
                                        <div className="text-xl sm:text-2xl font-bold">
                                            {hospital.total_appointments || 0}
                                        </div>
                                    </div>
                                    <div className="text-muted-foreground text-xs sm:text-sm">Appointments</div>
                                </div>
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                        <Star className="w-4 h-4 text-yellow-500" />
                                        <div className="text-xl sm:text-2xl font-bold">
                                            {hospital.rating?.average_rating?.toFixed(1) || '0.0'}
                                        </div>
                                    </div>
                                    <div className="text-muted-foreground text-xs sm:text-sm">
                                        Rating 
                                        <p className="inline-block ml-1 text-muted-foreground text-xs">
                                            ({hospital.rating?.total_reviews || '0'} review(s))
                                        </p>
                                    </div>
                                </div>
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                        ...
                                        <div className="text-xl sm:text-2xl font-bold">
                                            {hospital.total_patients || '0'}
                                        </div>
                                    </div>
                                    <div className="text-muted-foreground text-xs sm:text-sm">Patients Served</div>
                                </div>
                            </div>
                            ...
                            <div className="w-full mt-4 sm:mt-6 space-y-2 sm:space-y-3 text-left">
                                <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                                    ...
                                    <span className="break-words">{hospital.email}</span>
                                </div>
                                ...
                                {hospital.website && (
                                    <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                                        ...
                                        <span className="break-words">{hospital.website}</span>
                                    </div>
                                )}
                                ...
                                <div className="pt-2 border-t space-y-2">
                                    {hospital.bio && (
                                        <div className="text-xs">
                                            ...
                                            <span className="line-clamp-2">{hospital.bio}</span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="w-full mt-6 space-y-3 text-left">
                                ...
                                <div className="flex items-center justify-between gap-3 text-sm">
                                    ...
                                    <Button 
                                        ...
                                        onClick={() => handleViewDocument("Registration Certificate", hospital.license_document || "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf")}
                                    >
                                        View
                                ...
                                <div className="flex items-center justify-between gap-3 text-sm">
                                    ...
                                    <Button 
                                        ...
                                        size="sm" 
                                        className="bg-blue-600 hover:bg-blue-700 text-white cursor-pointer"
                                        onClick={() => handleViewDocument("Operating License", hospital.id_document || "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf")}
                                    >
                                        View
                                        <FileText className="w-4 h-4 text-white ml-1" />
                                    </Button>
                ...
                <div className="max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4 flex justify-between items-center">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                            ...
                            >
                                <span>Pending</span>
                                {hospital?.kyc_status === 'PENDING' && <Check className="w-4 h-4" />}
                            </DropdownMenuItem>
                            <DropdownMenuItem
                            ...
                            >
                                <span>Verified</span>
                                {hospital?.kyc_status === 'VERIFIED' && <Check className="w-4 h-4" />}
                            </DropdownMenuItem>
                            <DropdownMenuItem
                            ...
                            >
                                <span>Rejected</span>
                                {hospital?.kyc_status === 'REJECTED' && <Check className="w-4 h-4" />}
                            </DropdownMenuItem>
                            <DropdownMenuItem
                            ...
                            >
                                <span>Suspended</span>
                                {hospital?.kyc_status === 'SUSPENDED' && <Check className="w-4 h-4" />}
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
